import { useState, useRef, useEffect } from 'react';
import { ChevronDown, User, LogIn, UserPlus, UserCircle, LogOut, Users, Plus, UserRoundPlus } from 'lucide-react';

export default function NavBar({ isLoggedIn = false, onSignOut, userLeagues = [] }) {
  const [isLeaguesMenuOpen, setIsLeaguesMenuOpen] = useState(false);
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const leaguesMenuRef = useRef(null);
  const userMenuRef = useRef(null);

  // Close menus when clicking outside
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (leaguesMenuRef.current && !leaguesMenuRef.current.contains(event.target)) {
        setIsLeaguesMenuOpen(false);
      }
      if (userMenuRef.current && !userMenuRef.current.contains(event.target)) {
        setIsUserMenuOpen(false);
      }
    };

    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, []);

  const handleLeaguesMenuClick = (action, leagueId = null) => {
    setIsLeaguesMenuOpen(false);
    if (leagueId) {
      console.log(`Navigate to league: ${leagueId}`);
      // Handle league navigation
    } else {
      console.log(`League action: ${action}`);
      // Handle join/create actions
    }
  };

  const handleUserMenuClick = (action) => {
    setIsUserMenuOpen(false);
    if (action === 'sign-out' && onSignOut) {
      onSignOut();
    } else {
      console.log(`User menu action: ${action}`);
    }
  };

  const handleRecordGamesClick = () => {
    console.log('Navigate to Record Games');
    // Handle navigation to record games page
  };

  return (
    <nav className="navbar">
      <div className="navbar-container">
        {/* Logo/Brand */}
        <div className="navbar-left">
          <a href="/" className="navbar-brand">
            <span className="navbar-brand-text">Beach Kings</span>
          </a>
        </div>

        {/* Right side menu items */}
        <div className="navbar-right">
          {/* Record Games - No Dropdown */}
          <button
            className="navbar-menu-button"
            onClick={handleRecordGamesClick}
            aria-label="Record Games"
          >
            <svg
              width="20"
              height="20"
              viewBox="0 0 32 32"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="navbar-volleyball-icon"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M29 16C28.9979 14.5899 28.7712 13.2324 28.3539 11.9614C27.4315 11.5843 26.4757 11.3647 25.52 11.29L25.5399 11.34C27.1564 14.6719 27.8342 18.3167 27.5678 21.9377C28.4833 20.1579 29 18.1393 29 16ZM25.832 24.5051C26.735 20.2733 26.2082 15.8814 24.3 11.95L23.99 11.31C22.7299 11.41 21.49 11.78 20.37 12.37L20.9699 13.56C23.2565 18.2697 23.0285 23.7428 20.513 28.1953C22.5868 27.4276 24.4114 26.146 25.832 24.5051ZM18.5485 28.7503C21.5538 24.5015 22.0385 18.9062 19.74 14.17L19.21 13.1C18.19 13.84 17.31 14.81 16.64 15.97C17.8 18.5 17.93 21.33 17.02 23.97C16.74 24.79 16.37 25.55 15.92 26.26C15.88 26.37 15.82 26.47 15.74 26.54C15.1621 27.3856 14.4651 28.1418 13.6665 28.7911C14.4235 28.9283 15.2034 29 16 29C16.8722 29 17.7244 28.9141 18.5485 28.7503ZM11.9465 28.3556C12.6913 27.8755 13.357 27.2968 13.93 26.64C11.1094 26.3836 8.38542 25.5476 5.91755 24.207C7.47182 26.1141 9.5564 27.5719 11.9465 28.3556ZM4.23029 21.5276C7.31779 23.8114 11.0306 25.1471 14.8799 25.33C15.2199 24.77 15.51 24.17 15.73 23.53C15.94 22.92 16.09 22.28 16.17 21.65H15.82C10.7 21.65 5.95912 19.1448 3.03369 15.0769C3.01159 15.3851 3.00024 15.6963 3 16.01C3.00149 17.9829 3.44246 19.853 4.23029 21.5276ZM16 1C7.71573 1 1 7.71573 1 16C1 24.2843 7.71573 31 16 31C24.2843 31 31 24.2843 31 16C31 7.71573 24.2843 1 16 1ZM15.4499 15.34C17.9384 11.0035 23.03 9.02728 27.6846 10.3151C26.9446 8.80227 25.9195 7.45438 24.6792 6.34143C19.491 6.38884 14.6782 9.24078 12.17 13.87L11.84 14.46C12.94 14.98 14.1599 15.29 15.4499 15.34ZM10.97 13.2C13.4351 8.63216 17.9703 5.64964 23.012 5.07136C21.1647 3.88577 18.9976 3.15495 16.6688 3.03693C12.999 4.7261 9.94634 7.5135 7.92995 11.04C8.62995 12.13 9.55995 13.06 10.64 13.78L10.97 13.2ZM5.36526 8.54135C4.4232 9.87851 3.72695 11.4013 3.34222 13.044C5.88326 17.4798 10.6522 20.28 15.82 20.28H16.25C16.22 19.05 15.96 17.84 15.46 16.69C13.72 16.64 12.07 16.16 10.63 15.37C10.6 15.36 10.57 15.35 10.54 15.33C10.52 15.32 10.49 15.3 10.47 15.28C7.96507 13.8449 6.08806 11.4189 5.36526 8.54135ZM13.4298 3.27405C10.7129 3.81891 8.30201 5.21077 6.48444 7.16251C6.60169 8.02775 6.83748 8.8565 7.16995 9.63C8.78418 7.06439 10.9214 4.90622 13.4298 3.27405Z"
                fill="currentColor"
              />
            </svg>
            <span className="navbar-menu-label">Record Games</span>
          </button>

          {/* Leagues Menu */}
          <div className="navbar-menu-group" ref={leaguesMenuRef}>
            <button
              className="navbar-menu-button"
              onClick={() => setIsLeaguesMenuOpen(!isLeaguesMenuOpen)}
              aria-label="Leagues menu"
            >
              <Users size={20} />
              <span className="navbar-menu-label">Leagues</span>
              <ChevronDown 
                size={16} 
                className={`navbar-chevron ${isLeaguesMenuOpen ? 'open' : ''}`}
              />
            </button>

            {/* Leagues Dropdown Menu */}
            {isLeaguesMenuOpen && (
              <div className="navbar-dropdown navbar-dropdown-leagues">
                {/* User's Leagues (if logged in) */}
                {isLoggedIn && userLeagues && userLeagues.length > 0 && (
                  <>
                    <div className="navbar-dropdown-section">
                      <div className="navbar-dropdown-header">My Leagues</div>
                      {userLeagues.map((league) => (
                        <button
                          key={league.id}
                          className="navbar-dropdown-item navbar-dropdown-item-league"
                          onClick={() => handleLeaguesMenuClick('view-league', league.id)}
                        >
                          <Users size={18} />
                          {league.name}
                        </button>
                      ))}
                    </div>
                    <div className="navbar-dropdown-divider"></div>
                  </>
                )}
                
                {/* Action Items */}
                <div className="navbar-dropdown-section">
                  <button
                    className="navbar-dropdown-item"
                    onClick={() => handleLeaguesMenuClick('join-league')}
                  >
                    <UserRoundPlus size={18} />
                    Join League
                  </button>
                  <button
                    className="navbar-dropdown-item"
                    onClick={() => handleLeaguesMenuClick('create-league')}
                  >
                    <Plus size={18} />
                    Create League
                  </button>
                </div>
              </div>
            )}
          </div>

          {/* User Menu */}
          <div className="navbar-menu-group" ref={userMenuRef}>
            <button
              className="navbar-menu-button"
              onClick={() => setIsUserMenuOpen(!isUserMenuOpen)}
              aria-label="User menu"
            >
              <User size={20} />
              <span className="navbar-menu-label">Account</span>
            </button>

            {/* User Dropdown Menu */}
            {isUserMenuOpen && (
              <div className="navbar-dropdown navbar-dropdown-user">
                {!isLoggedIn ? (
                  <>
                    <button
                      className="navbar-dropdown-item"
                      onClick={() => handleUserMenuClick('sign-up')}
                    >
                      <UserPlus size={18} />
                      Sign Up
                    </button>
                    <button
                      className="navbar-dropdown-item"
                      onClick={() => handleUserMenuClick('sign-in')}
                    >
                      <LogIn size={18} />
                      Sign In
                    </button>
                  </>
                ) : (
                  <>
                    <button
                      className="navbar-dropdown-item"
                      onClick={() => handleUserMenuClick('profile')}
                    >
                      <UserCircle size={18} />
                      My Profile
                    </button>
                    <button
                      className="navbar-dropdown-item navbar-dropdown-item-danger"
                      onClick={() => handleUserMenuClick('sign-out')}
                    >
                      <LogOut size={18} />
                      Sign Out
                    </button>
                  </>
                )}
              </div>
            )}
          </div>
        </div>
      </div>
    </nav>
  );
}


